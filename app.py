...
def generate_grad_cam(model, img_array, class_index, img_size, img, uploaded_file):
    ...
    superimposed_img = heatmap_rgb * 0.4 + original_img * 0.6
...
def main():
    ...
    # Only load the model(s) actually needed for the current selection.
    # Cached calls are cheap on rerun, but we still avoid loading both
    ...
    def process_scan(uploaded_file):
        if selected_model == "Ensemble (Xception + CNN)":
            assert model_xc is not None and model_cnn is not None, "Ensemble models not loaded."
